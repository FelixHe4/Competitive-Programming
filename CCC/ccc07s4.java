...
public class ccc07s4 {
	public static int num_nodes, x, y, cnt[];
	...
	public static void main(String[] args) throws IOException {
		num_nodes = readInt();
		connections = new ArrayList[num_nodes + 1];
		for (int i = 0; i <= num_nodes; i++) {
			connections[i] = new ArrayList<Integer>();
		}
		cnt = new int[num_nodes + 1];
		vis = new boolean[num_nodes + 1];
		x = readInt();
		y = readInt();
		vis[1] = true;
		cnt[1] = 1;
		...
		dfs(num_nodes, -1);
		System.out.println(cnt[num_nodes]);
	}
