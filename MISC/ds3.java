...
public class ds3 {
	...
	static void Build(long l, long r, int root) {
		...
		if (l == r) {
			...
			seg[root].min = Math.min(seg[(root << 1)].min, seg[root << 1 | 1].min);
			seg[root].gcd = gcd(seg[root << 1].gcd, seg[root << 1 | 1].gcd);
			if (seg[root].gcd == seg[root << 1].gcd) {
				seg[root].sum += seg[root << 1].sum;
			}
			if (seg[root].gcd == seg[root << 1 | 1].gcd) {
				seg[root].sum += seg[root << 1 | 1].sum;
			}
		}
	}
	...
	static long Minimum(long l, long r, long low, long high, int root) {
		...
		long mid = (low + high) / 2;
		return Math.min(Minimum(l, r, low, mid, root << 1), Minimum(l, r, mid + 1, high, root << 1 | 1));
	}
	...
	public static long GCD(long l, long r, int pos) {
		if (seg[pos].l >= l && seg[pos].r <= r) {
			...
			if (r <= mid) {
				return GCD(l, r, pos << 1);
			} else if (l > mid) {
				return GCD(l, r, pos << 1 | 1);
			} else {
				return gcd(GCD(l, r, pos << 1), GCD(l, r, pos << 1 | 1));
			}
		}
	}
