...
public class ds4 {
	...
	// get the Index of a value v
	public static int getIndex(Node n, int v) {
		if (n != null) {
			if (n.key >= v) {
				return getIndex(n.left, v);
			} else {
				return 1 + getSize(n.left) + getIndex(n.right, v);
			}
		}
		return 0;
	}

	// check if a value v is inside the AVL Tree
	...
	public static Node remove(Node n, int k) {
		...
		if (k > n.key) {
			...
			// this node to remove;
			if (n.left == null) {
				// if there is no node to the left of the node being removed, change the node being removed to the node on the right side.
				return n.right;
				...
				// n will become the left most node.
				// the right side of n will find the left left most node, it will be replaced with the node on the right side,
	...
	public static void main(String[] args) throws IOException {
		...
		while (m-- > 0) {
			...
			if (s.equals("I")) {
				...
				lastAns = smallestElement(root, v - 1);
				println(lastAns);
			} else {
				if (contains(root, v)) {
					lastAns = getIndex(root, v) + 1;
					println(lastAns);
				} else {
					lastAns = -1;
					println(-1);
